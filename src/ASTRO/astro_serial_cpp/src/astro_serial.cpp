...
public:
  AstroSerialNode() : Node("astro_serial_node"), uart_thread_running_(true)
  {
    ...
    // Ovi parametri su izvor istine za IK pravog robota.
    // Sim (diff_drive_controller + xacro) MORA koristiti iste vrijednosti,
    // inače isti /cmd_vel daje drugačiji odziv. Nema Twist limitera:
    // saturacija je samo clamp kotača na max_wheel_angular_velocity.
...
private:
  ...
  std::vector<double> diff_drive_inv_kinematics(geometry_msgs::msg::Twist twist_msg)
  {
    ...
    // Saturacija je na kotačima, NE na Twist-u. Ako v i w zajedno
    // traže |w_wheel| > max, clamp mijenja ostvareni v/w omjer
    // (radijus skretanja). Sim to mora raditi isto, bez v/w limita.
